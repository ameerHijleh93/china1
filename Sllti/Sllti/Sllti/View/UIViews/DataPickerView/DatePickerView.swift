...
protocol DatePickerViewDelegate: class {
    ...
    func dateView(_ dateView: DatePickerView, didSelect date: Date)
    func dateViewDidCancelSelection(_ dateView: DatePickerView)
...
class DatePickerView: UIView {
    ...
    struct Config {
        ...
        var headerBackgroundColor: UIColor = UIColor.gray
    ...
    @objc func confirmButtonDidTapped(_ sender: AnyObject) {
        ...
        dismiss()
        delegate?.dateView(self, didSelect: datePicker.date)
    }
    
    @objc func cancelButtonDidTapped(_ sender: AnyObject) {
        dismiss()
        delegate?.dateViewDidCancelSelection(self)
    }
    ...
    // MARK: - Private
    fileprivate func setup(_ parentVC: UIViewController, type: UIDatePickerMode) {
    ...
    // MARK: - Public
    func show(inVC parentVC: UIViewController, type: UIDatePickerMode,completion: (() -> ())? = nil) {
        
        setup(parentVC, type: type)
        move(goUp: true)
